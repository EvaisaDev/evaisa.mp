...
function hub_create(name, steam_id)
    local http = hub_http()
    local data, err = http.post("/hub/create", {
        owner_steam_id = tostring(steam_id),
        name = name
    })
    if err or not data then return nil, err or "failed" end
    return data, nil
end

function hub_join_by_code(invite_code, steam_id)
    local http = hub_http()
    local data, err = http.post("/hub/join", {
        invite_code = invite_code,
        steam_id = tostring(steam_id)
    })
    if err or not data then return nil, err or "failed" end
...
function hub_leave()
    if hub_state == nil then return end
    local http = hub_http()
    http.post("/hub/" .. hub_state.id .. "/leave", {}, hub_auth())
    hub_state = nil
    hub_token = nil
    hub_source_lobby = false
    hub_spectator_mode = false
...
function hub_heartbeat()
    if hub_state == nil then return end
    local http = hub_http()
    local data, err = http.post("/hub/" .. hub_state.id .. "/heartbeat", {}, hub_auth())
    if data then
        if data.online_members then hub_state.online_members = data.online_members end
        if data.lobbies then hub_state.lobbies = data.lobbies end
        if data.feed then hub_state.feed = data.feed end
...
function hub_refresh()
    if hub_state == nil then return nil, "not_in_hub" end
    local http = hub_http()
    local data, err = http.get("/hub/" .. hub_state.id, hub_auth())
    if data then
        hub_state.online_members = data.online_members or {}
        hub_state.lobbies = data.lobbies or {}
        hub_state.feed = data.feed or {}
...
function hub_create_lobby(name, gamemode, settings)
    if hub_state == nil then return nil, "not_in_hub" end
    local http = hub_http()
    local data, err = http.post("/hub/" .. hub_state.id .. "/lobby", {
        name = name,
        gamemode = gamemode or "",
        settings = settings or {}
    }, hub_auth())
    ...
    hub_state.lobbies = hub_state.lobbies or {}
    hub_state.lobbies[data.lobby.token_id] = data.lobby
    return data.lobby, nil
end

function hub_delete_lobby(token_id)
    if hub_state == nil then return nil, "not_in_hub" end
    local http = hub_http()
    local data, err = http.delete("/hub/" .. hub_state.id .. "/lobby/" .. token_id, hub_auth())
    if not err and hub_state.lobbies then
        hub_state.lobbies[token_id] = nil
    end
    return data, err
...
function hub_update_lobby_state(token_id, fields)
    if hub_state == nil then return nil, "not_in_hub" end
    local http = hub_http()
    local data, err = http.put("/hub/" .. hub_state.id .. "/lobby/" .. token_id .. "/state", fields, hub_auth())
    if data and data.lobby and hub_state.lobbies then
        hub_state.lobbies[token_id] = data.lobby
    end
    return data, err
...
function hub_activate_lobby(token_id, steam_lobby_id)
    return hub_update_lobby_state(token_id, { steam_lobby_id = tostring(steam_lobby_id) })
end

function hub_set_lobby_in_progress(token_id, in_progress)
    return hub_update_lobby_state(token_id, { in_progress = in_progress })
end

function hub_add_moderator(steam_id)
    if hub_state == nil then return nil, "not_in_hub" end
    local http = hub_http()
    local data, err = http.post("/hub/" .. hub_state.id .. "/moderator", { steam_id = tostring(steam_id) }, hub_auth())
    if data then hub_state.moderators = data.moderators or hub_state.moderators end
    return data, err
end

function hub_remove_moderator(steam_id)
    if hub_state == nil then return nil, "not_in_hub" end
    local http = hub_http()
    local data, err = http.delete("/hub/" .. hub_state.id .. "/moderator/" .. tostring(steam_id), hub_auth())
    if data then hub_state.moderators = data.moderators or hub_state.moderators end
    return data, err
end

function hub_update_settings(settings)
    if hub_state == nil then return nil, "not_in_hub" end
    local http = hub_http()
    local data, err = http.put("/hub/" .. hub_state.id .. "/settings", settings, hub_auth())
    if data then hub_state.settings = data.settings or hub_state.settings end
    return data, err
end
...
function hub_report_feed(event_type, lobby_name, winner, score, extra)
    if hub_state == nil then return end
    local http = hub_http()
    http.post("/hub/" .. hub_state.id .. "/feed", {
        event_type = event_type,
        lobby_name = lobby_name or "",
        winner = winner or "",
        score = score or "",
...
function hub_delete(hub_id)
    if hub_state == nil then return nil, "not_in_hub" end
    local http = hub_http()
    local data, err = http.delete("/hub/" .. hub_id, hub_auth())
    if not err then
        hub_state = nil
        hub_token = nil
        hub_source_lobby = false
